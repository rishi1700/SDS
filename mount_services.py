...
SN_VOLUME_MOUNT_PATH = "/mnt/"

AWS_TEST = False

def get_mount_base_path():
    if os.path.exists(SN_VOLUME_MOUNT_PATH):
        if os.access(SN_VOLUME_MOUNT_PATH, os.W_OK | os.X_OK):
            return SN_VOLUME_MOUNT_PATH
    return os.getcwd()
...
def is_mounted(path):
    try:
        if sys.platform.startswith("darwin"):
            ...
            # os.path.ismount is reliable on Linux; fall back to /proc/mounts
            # for stale NFS mounts where ismount raises OSError (ESTALE).
            try:
...
def mount_iscsi_chap(remote_ip, local_mnt_path, iqn, user, password, volume_name, wait_time):
    ...
    try:
        # Windows mount
        if sys.platform.startswith("win"):
            # Session established by ShowiSCSIChapMount; trigger rescan so
            # Windows enumerates the new disk before the GUI polls Get-Disk.
...
def mount_iscsi_nochap(remote_ip, local_mnt_path, iqn, wait_time):
    ...
    try:
        if sys.platform.startswith("win"):
            subprocess.check_output(["iscsicli", "QLoginTarget", iqn])
            time.sleep(int(wait_time))
...
def mount_process(volumeName, protcol_name,remote_ip, user_name, ip, password):
    if AWS_TEST:
        # Create Folder and Give 777 permission
        mount_path = SN_VOLUME_MOUNT_PATH + volumeName
...
def unmount_process(volumeName, remote_ip, protocol):
    try: 
        if AWS_TEST:
            # Remove persmissions for volume
            un_mount_path = os.path.join(SN_VOLUME_MOUNT_PATH, volumeName)
            
            if not os.path.exists(un_mount_path):
                return -1, f"Path does not exist: {un_mount_path}"
            
            os.chmod(un_mount_path, 0o555)
            return 0, un_mount_path
...
def deleteVolumeFolder(path, protocol_name):
    try:
        if AWS_TEST:
            shutil.rmtree(path)
            return 0
...
def mountVolume(volumeName, protocol_name, remote_ip, user_name, ip, password):
    try:
        result, mount_path, error_message = mount_process(volumeName, protocol_name, remote_ip,  user_name, ip, password)
        if result == 0:
...
def unmountVolume(volumeName, remote_ip, protocol_name):
    try:
        result, unmount_path = unmount_process(volumeName, remote_ip, protocol_name)
        if result == 0:
...
def deleteFolder(volumeName, protocol_name):
    try:
        if sys.platform.startswith("win"):
            local_path = find_window_drive_by_volume(volumeName,protocol_name)
            response_data = {"status": "success", "message": "Drive deleted successfully.", "local_path" : local_path}
        else:
            Mount_Path = get_mount_base_path()
            local_path=os.path.join(Mount_Path, volumeName)

            if not os.path.exists(local_path):
                response_data = {"status": "failure", "message": "Volume Folder not found to delete", "local_path" : local_path}
                return response_data

            result = deleteVolumeFolder(local_path, protocol_name)
            if result == 0:
