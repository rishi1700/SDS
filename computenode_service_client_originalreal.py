...
def mountVolume():
    try:
        ...
        result, mount_path = mount_process(volumeName, protocol_name, remote_ip, host_name,  user_name, ip, password, wwn, url)
        if result == 0:
            response_data = {"status": "success", "message": "Mount process completed successfully.", "mount_path" : mount_path}
        else:
            response_data = {"status": "failure", "message": "Mount process failed.", "mount_path" : mount_path}
        return response_data
    except Exception as e:
        response_data = {"status": "failure", "message": str(e), "mount_path" : "N/A"}
        return response_data
